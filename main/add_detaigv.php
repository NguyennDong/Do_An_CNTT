<?php 
     ...
     if (isset($_POST['action'])){
        ...
        if($action == 'add_topic') {
            ...
            else if ($query_loai_detai == false){
                echo $show_alert.'Chưa có loại đề tài';
            }
            else {
        ...
        if($action == 'edit_topic'){
            ...
            $show_alerxt = '<script>$("#formEditTopic .alert").removeClass("hidden");</script>';
            ...
            $query_edit = query("UPDATE de_tai SET tenDT = '$name_edit_topic', moTa = '$desc_edit_topic', loaiDT = '$cate_edit_topic' WHERE id =  '$id_edit_topic'");
            confirm($query_edit);
                if ($query_edit){
